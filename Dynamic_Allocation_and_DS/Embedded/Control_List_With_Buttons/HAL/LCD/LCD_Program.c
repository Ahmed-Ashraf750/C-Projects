...
void hLCD_SendCommand(uint8_t Command) // to send a command we clear rs pin and send the command to the lcd
{
...
void hLCD_SetCursor(uint8_t Line, uint8_t Digits)
{
    ...
    hLCD_SendCommand(DDRAM_Address | SET_DDRAM);
    _delay_ms(1);
}

void hLCD_WriteChar(char Character) // to send data we set rs pin and send the data to the lcd
{
...
// you can store up to 8 custom characters in the CGRAM of the LCD. Each character is defined by an array of 8 bytes, where each byte represents a row of the character (5x8 pixels). The function below allows you to store a custom character pattern in a specific CGRAM location (0-7).


void hLCD_StoreSpecialChar(uint8_t *Pattern, uint8_t Location)
{
    if (Pattern == Null || Location > 7)
    {
    ...
    hLCD_SendCommand(0x40 | CGRAM_Address);
    _delay_ms(1);

    for (uint8_t i = 0; i < 8; i++)
    {
        hLCD_WriteChar(Pattern[i]);
        _delay_ms(1);
    }

    hLCD_SendCommand(0x80); // Return to DDRAM mode
    _delay_ms(1);
}
