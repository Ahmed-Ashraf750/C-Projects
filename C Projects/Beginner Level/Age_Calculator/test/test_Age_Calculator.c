 ...
 * edge case: if the birth date is the same as the current date, the age should be 0 years, 0 months, and 0 days. (done)
 ...
 * edge case: comparing two empty strings should return a match. (done)
 ...
 * edge case: entering an invalid date format should be handled gracefully without crashing the program. (done)
...
void test_calculate_Age_Null_Pointer_Should_Handle_Gracefully(void)
{
    Date_t today = {15, 8, 2023};
...
void test_Compare_Strings_Empty_Strings_Should_Return_Match(void) // Compare two empty strings should return a match. it fails because the function does not handle empty strings correctly -> Compare_Strings should check for empty strings and return Matches if both are empty
{
    const char *str1 = "";
    const char *str2 = "";
...
// Tests for Scan_Date
void test_Scan_Date_Invalid_Format_Should_Handle_Gracefully(void) // entering an invalid date format should be handled gracefully without crashing the program. it fails because the function does not handle invalid formats correctly -> Scan_Date should validate the input format and return Fail if it's invalid
{
    Date_t present_date;
...
void test_Scan_Date_Null_Pointer_Should_Handle_Gracefully(void) // passing a null pointer for either Present_Date or Date_Of_Birth should be handled gracefully. it fails because the function does not check for null pointers -> Scan_Date should check for null pointers and return Fail if either is null
{
    Date_t present_date;
    Date_t date_of_birth;
